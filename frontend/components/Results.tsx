...
function Title(query: string, numberOfResults: number, totalDocuments: number, calculationTime: number) {
    return (
        <div className="italic text-justify m-10 mb-0 flex">
            <div className="border-0 rounded-lg p-5 pl-0 pb-2">
            ...
            <div className='grid gap-4 grid-cols-1 px-0 md:px-10'>
                {
                    props.rawResults?.map(
                        (result: Result) => <Card key={result.id} result={result} searchForSimilar={props.searchForSimilar} />
                    )
                }
            </div>
        </div>
